...
export default function Adjustable({ children }: { children: React.ReactNode }) {
    ...
    if (phone) {
        ...
        return (
            <div className={`${navbar ? 'ml-72' : 'ml-24'} trans-cubic-second`}
                style={{ width: `calc(100% - ${navbar ? "288px" : "96px"})`, height: "100%", marginTop: router.asPath.includes("/connect") ? "0px" : "56px" }}
            >
                {children}
            </div>
        )
